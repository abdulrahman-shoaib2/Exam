...
export default class HolidaysUI extends Component {
  ...
  errorState() {
    ...
    <div class="empty-state">
        ...
        <p>No public holidays found for ${Destination.yearSelectedOption}</p>
      </div>`;
  }
  emptyState() {
    // document.getElementById('holidays-content').innerHTML=
  ...
  withDataState(list) {
    document.getElementById("holidays-content").innerHTML = list
      .map((card) => {
  ...
  initCardPlanBtn(list) {
    Array.from(document.querySelectorAll(".holiday-action-btn ")).map(
      (btn, index) => {
